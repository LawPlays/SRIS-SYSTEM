...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-3xl font-bold text-white">Edit Announcement</h1>
    </div>
        ...
        <form method="POST" action="{{ route('teacher.announcements.update', $announcement) }}">
            ...
            <div class="mb-6">
                ...
                <select id="priority" 
                        ...
                        required>
                    <option value="1" {{ old('priority', $announcement->priority) == '1' ? 'selected' : '' }}>Low</option>
                    <option value="2" {{ old('priority', $announcement->priority) == '2' ? 'selected' : '' }}>Medium</option>
                    <option value="3" {{ old('priority', $announcement->priority) == '3' ? 'selected' : '' }}>High</option>
                </select>
                @error('priority')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
